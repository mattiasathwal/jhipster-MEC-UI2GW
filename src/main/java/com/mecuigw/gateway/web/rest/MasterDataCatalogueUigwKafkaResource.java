...
@RestController
@RequestMapping("/api/master-data-catalogue-uigw-kafka")
public class MasterDataCatalogueUigwKafkaResource {

    private final Logger log = LoggerFactory.getLogger(MasterDataCatalogueUigwKafkaResource.class);

    private final MessageChannel output;
    private Sinks.Many<Message<String>> sink = Sinks.many().unicast().onBackpressureBuffer();

    public MasterDataCatalogueUigwKafkaResource(@Qualifier(KafkaSseProducer.CHANNELNAME) MessageChannel output) {
        this.output = output;
    }
